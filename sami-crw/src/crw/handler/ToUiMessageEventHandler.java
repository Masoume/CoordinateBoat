...
import crw.uilanguage.message.toui.AreaSelectionMessage;
...
public class ToUiMessageEventHandler implements EventHandlerInt {
    ...
    public void invoke(final OutputEvent oe, ArrayList<Token> tokens) {
        ...
        if (oe instanceof OperatorPathOptions) {
            ...
            message = new MethodOptionMessage(oe.getId(), oe.getMissionId(), priority, methods);
        ...
        /*else if (oe instanceof OperatorSelectDangerousArea){
           ...
           // ArrayList<Area2D> areas = new ArrayList<Area2D>();
              Area2D area = ((OperatorSelectDangerousArea) oe).getArea();
           // areas.add(area);
            for (Location location : area.getPoints()) {
                positions.add(location);
            }      
            LOGGER.log(Level.SEVERE, "before sending message " + area, this);
            message = new AreaSelectionMessage(oe.getId(), oe.getMissionId(), priority,positions);
